...
(defn container []
  (let [current-replicas (:current-replicas @variables)
        ...
        current-average-metric (:current-average-metric @variables)
        [color phrase] (replicas-color-phrase current-replicas desired-replicas)]
    [:section {:class "mw5 mw7-ns center bg-light-gray pa3 ph5-ns"}
     [:h3 {:class "tc"}
      [:span {:class color} phrase]]
     [:h3 {:class "tc"} (str "Desired Replicas: " desired-replicas)]
     [:h3 {:class "tc"} (str "Current Avg Request Count: " current-average-metric)]
     [slider "Current Replicas" :current-replicas 1 30]
     [slider "Current Request Count" :current-metric 0 350]
